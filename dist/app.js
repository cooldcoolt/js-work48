"use strict";
const BASE_URL = "http://146.185.154.90:8000/blog/";
const MAIL = "cooldcoolt@gmai.com";
// Константы для конфигурации
const POLLING_INTERVAL = 3000; // Интервал опроса новых сообщений (мс)
const MESSAGE_REFRESH_DELAY = 1500; // Задержка обновления после отправки (мс)
const MAX_MESSAGES_TO_SHOW = 20; // Максимальное количество отображаемых сообщений
const MESSAGE_LIMIT_THRESHOLD = 19; // Порог для ограничения сообщений
let lastMessageDate;
...
const loader = document.querySelector(".loader-cn");
...
const builUserInfo = async (baseUrl, method) => {
    var _a, _b;
    try {
        ...
        let leter = ((_b = (_a = profileOb.firstName) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.toUpperCase()) || "U";
...
const controlUser = async () => {
    ...
    let form = overlay.querySelector(".login");
    let inputName = form.querySelector(".inputName");
    let inputLastName = form.querySelector(".inputLastName");
    let button = form.querySelector(".inputButton");
    let exitButton = form.querySelector(".exitButton");
    ...
    const newInputName = inputName.cloneNode(true);
    const newInputLastName = inputLastName.cloneNode(true);
...
const buildInput = () => {
    ...
    let messagePost = document.createElement("div");
    messagePost.classList.add("messagePost");
    let messagePostForm = document.createElement("form");
...
const buildMessage = async (url) => {
    var _a, _b, _c, _d, _e, _f, _g;
    try {
        const message = await postMessage12(url, "GET", null);
        let curentUsr = null;
        let profileStr = localStorage.getItem("profile");
        if (profileStr) {
            try {
                curentUsr = JSON.parse(profileStr);
            }
            catch (e) {
        ...
        let messageBlock = mainsec.querySelector(".messageBlock");
        if (!messageBlock) {
        ...
        const wasScrolledToBottom = messageBlock.scrollHeight - messageBlock.scrollTop <= messageBlock.clientHeight + 50;
        ...
        for (let i = message.length - 1; i >= startIndex; i--) {
            ...
            messageDiv.setAttribute("email", ((_a = message[i].user) === null || _a === void 0 ? void 0 : _a.email) || "");
            messageDiv.setAttribute("data-message-id", msgId);
            if (((_b = message[i].user) === null || _b === void 0 ? void 0 : _b.email) === (curentUsr === null || curentUsr === void 0 ? void 0 : curentUsr.email)) {
                messageDiv.style.marginLeft = "auto";
                messageDiv.classList.add("own-message");
            }
            const ava1 = document.createElement("strong");
            ava1.classList.add("ava1");
            const firstName = ((_c = message[i].user) === null || _c === void 0 ? void 0 : _c.firstName) || "U";
            const lt = ((_d = firstName[0]) === null || _d === void 0 ? void 0 : _d.toUpperCase()) || "U";
            ava1.textContent = lt;
            ...
            const fullName = `${((_e = message[i].user) === null || _e === void 0 ? void 0 : _e.firstName) || ""} ${((_f = message[i].user) === null || _f === void 0 ? void 0 : _f.lastName) || ""}`.trim() || "Неизвестный пользователь";
        ...
        if (message.length > 0) {
            lastMessageDate = ((_g = message[message.length - 1]) === null || _g === void 0 ? void 0 : _g.datetime) || lastMessageDate;
        }
    }
    catch (error) {
...
const postMessage12 = async (url, method, searchParams) => {
    try {
...
const subscribe = async () => {
    let overlay = root.querySelector(".overlay2");
    if (!overlay)
        return;
    overlay.style.display = "flex";
    let form = overlay.querySelector(".login");
